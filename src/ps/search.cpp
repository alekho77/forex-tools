...
std::tuple<double, double, double> operator*(const std::tuple<double, double, double>& a,
...
// double SubSearch(fxlib::IForecaster* forecaster, const fxlib::fxsequence& seq, const fxlib::ForecastInfo& info,
// double& profit, double& loss, double& threshold) {
//  using namespace std;
//  auto fun = [&](double p, double l, double t) {
//    cout << ".";
//    return Play(forecaster, seq, info.position, info.timeout, info.window, p, l, t, nullptr, nullptr, nullptr,
//    nullptr, nullptr, nullptr);
//  };
//  double total = fun(profit, loss, threshold);
//  cout << fixed << setprecision(4) << " point [" << (profit / g_pip) << "," << (loss / g_pip) << "," << threshold <<
//  "]: " << (total / g_pip) << endl; auto grad = Grad(fun, profit, loss, threshold); auto dp = g_rate * grad; auto pt =
//  make_tuple(profit, loss, threshold) + dp; double totalt = fun(get<0>(pt), get<1>(pt), get<2>(pt)); double eps =
//  abs(total - totalt); auto out = [&]() {
//    cout << fixed << setprecision(4) << "(" << grad << ")->[" << (profit / g_pip) << "," << (loss / g_pip) << "," <<
//    threshold << "]: " << (total / g_pip) << " => " << (eps / g_pip) << endl;
//  };
//  profit = get<0>(pt);
//  loss = get<1>(pt);
//  threshold = get<2>(pt);
...
void Search(const boost::property_tree::ptree& prop) {
    ...
    cout << "Searching best play params for algorithm " << g_algname << " in threshold range "
         << get<0>(g_threshold_range) << "-" << get<1>(g_threshold_range) << endl;
    ...
    uniform_real_distribution<double> dist_profit(get<0>(g_take_profit_range) * g_pip,
                                                  get<1>(g_take_profit_range) * g_pip);
    ...
    do {
        ...
        cout << setprecision(4) << "    candidate ["
             << make_tuple(profit_cand / g_pip, loss_cand / g_pip, threshold_cand) << "]: ";
        ...
        auto jump = [&](bool undoubted) {
            ...
            total = total_cand;
            cout << setprecision(4) << "Point [" << make_tuple(profit / g_pip, loss / g_pip, threshold)
                 << "]: " << setprecision(2) << total / g_pip << endl;
        };
        if (dh < 0) {
            jump(true);
        } else {
    ...
    double sum_loss;
    double sum_timeout;
    Play(&*forecaster, seq, info.position, info.timeout, info.window, profit, loss, threshold, &N, &Np, &sum_profit,
         &Nl, &sum_loss, &sum_timeout);
