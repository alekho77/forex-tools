...
class fxquote_test_fixture : public ::testing::Test {
 protected:
    const fxsequence corr_sequence = {
        boost::posix_time::minutes(1),
        date_period(date(2015, Jan, 1), days(31)),
        {
    ...
    const detail::fxsequence_header_bin test_header = {
        1, 1234, {{0x20, 0x15, 0x01, 0x01, 0xDD, 0xFF, 0x00, 0x00}, {0x20, 0x15, 0x02, 0x01, 0xDD, 0x00, 0x00, 0x00}}};
    const detail::fxcandle_bin test_candle = {
        {0x20, 0x15, 0x01, 0x15, 0xDD, 0x15, 0x15, 0xFF}, 1123450, 1543210, 1555550, 1111110, 123};
};
...
TEST_F(fxquote_test_fixture, fxsequence_serialize) {
    ...
    buf_type buf;
    {
        ...
        ASSERT_FALSE(out.fail());
        ASSERT_EQ(sizeof(detail::fxsequence_header_bin) + corr_sequence.candles.size() * sizeof(detail::fxcandle_bin),
                  buf.size());
    }
    {
